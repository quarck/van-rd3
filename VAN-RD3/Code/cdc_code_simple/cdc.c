...
#elif defined (DEVICE_REV_3)
#  define USART_UBBR_VALUE 8 /* Really, not using UART for REV_3 */
#else 
...
#elif defined DEVICE_REV_3
	DDRC |= (1<<PC0) | (1<<PC1) | (1<<PC2) | (1<<PC3) | (1<<PC4) ;/* set PC0-PC4 to output, PC5 is switch button, input */
#else 
# error "unknown device"
#endif
	led_show_num(0xff);
...
#define PC5BtnPressed() ((PINC & ((uint8_t)(1<<PC5))) != 0)
